...
pathway_id <- "hsa04940" #to be removed, can be called
pathway_data <- keggGet(pathway_id)
pathway_genes <- pathway_data[[1]]$GENE
gene_ids <- pathway_genes[seq(1, length(pathway_genes), 2)] #to be used by Lucy
gene_symbols <- sapply(pathway_genes, function(x) strsplit(x, ";")[[1]][1]) #to be used for labels in visualisation
...
#this works, it returns a list of all available human pathways
